...
ensure_homebrew_taps() {
	...
	# 字体 cask 已迁移到 Homebrew Cask 主仓库，无需额外 tap。
	return 0
}
...
configure_package_manager() {
	...
	dnf)
		...
		if grep -q "max_parallel_downloads" "$DNF_CONF" 2>/dev/null; then
			...
			info "[DRY-RUN] 将备份并优化 DNF 配置: $DNF_CONF"
		else
			info "备份并优化 DNF 配置..."
...
EOF
	...
	apt)
		...
		if [ ! -f "$APT_CONF" ] || ! grep -q "Acquire::Queue-Mode" "$APT_CONF" 2>/dev/null; then
			if [ "$DRY_RUN" = true ]; then
				info "[DRY-RUN] 将配置 APT 并行下载: $APT_CONF"
			else
				info "配置 APT 并行下载..."
...
_install_vscode_fedora() {
	...
	if ! ${SUDO} rpm --import "$rpm_key"; then
		warn "导入 GPG 密钥失败"
		rm -f "$rpm_key"
		return 1
	fi
...
deploy_config_files() {
	...
	for src in "${!CONFIG_FILES_COPY[@]}"; do
		...
		if ! _deploy_config_item "$src" "$dst" "false"; then
			deploy_failed=true
		fi
	done
	...
	for src in "${!CONFIG_DIRS_COPY[@]}"; do
		...
		if ! _deploy_config_item "$src" "$dst" "true"; then
			deploy_failed=true
		fi
	done
